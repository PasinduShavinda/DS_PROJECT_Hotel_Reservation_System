...
export default class Shv_All_Taxi_Reservations extends Component {
    ...
    onDelete = (id) =>{
      ...
      .then((willDelete) => {
        if (willDelete) {
          axios.delete(`http://localhost:8001/postTaxi/delete/${id}`).then((res)=>{
            this.retrieveTaxiPosts();
            })
  ...
  render() {
    return (
        ...
        <button className = "btn btn-success"><a href = "#" style ={{textDecoration:'none', color:'white'}}></a></button>
            ...
            {this.state.TaxiPosts.map((TaxiPosts, index) =>(
              ...
              <tr key = {index}>
                <th scope = "row">{index+1}</th>
                    ...
                    <td>
                    <a className = "btn btn-primary" href = {`/ViewTaxi/${TaxiPosts._id}`}>
                    View
                    </a>
                    </td>

                    <td>  
                    <a className = "btn btn-warning" href = {`/UpdateTaxi/${TaxiPosts._id}`}>
                    Edit
                    </a>
                    </td>

                    <td>
                    <a className = "btn btn-danger" onClick={ event => this.onDelete(TaxiPosts._id)} >
                      Delete
                    </a>
                    </td>
